    ...
    Renderer: text::Renderer,
{
    ...
    /// Eases whole mouse-wheel steps. Pixel scrolling, fractional
    /// line gestures, and their subsequent whole-line packets remain immediate, as do touch,
    /// scrollbar dragging, and programmatic scrolling.
    ...
    Renderer: text::Renderer,
{
    ...
    fn update(
        ...
        // Advance before dispatching to children so their hit testing uses the
        // same offset as this frame's rendering, including inside overlays.
        ...
        {
            state.smooth_motion = None;
        }
        if let Event::Window(window::Event::RedrawRequested(now)) = event {
            if state.advance_smooth_scroll(*now, bounds, content_bounds) {
                let _ = notify_scroll(state, &self.on_scroll, bounds, content_bounds, shell);
            }
            if state.smooth_motion.is_some() {
        ...
        let mut update = || {
            ...
            match event {
                Event::Mouse(mouse::Event::WheelScrolled { delta }) => {
                    ...
                    let animate = self.smooth_scroll
                        && state.wheel_input.should_animate(*delta, Instant::now());
                    ...
                    if animate {
                        if state.queue_smooth_scroll(delta, bounds, content_bounds, Instant::now())
                        {
                            state.last_scrolled = Some(Instant::now());
                            shell.capture_event();
                            shell.request_redraw();
                        } else if state.last_scrolled.is_some() {
                            shell.capture_event();
                        }
                        return;
                    }
                    // Precise input takes over from the displayed position.
                    // Flushing the pending wheel target here would cause a jump.
...
/// pattern. Delta units alone do not identify a device: Windows touchpads also
/// produce line deltas. Fractional and pixel input stay direct, including later
/// whole-line packets within that gesture. Event timing does not establish
/// precision: fast wheel notches and batched events must remain animated.
#[derive(Debug, Default, Clone, Copy, PartialEq)]
pub struct WheelScrollInput {
    last_event: Option<Instant>,
    continuous: bool,
}

impl WheelScrollInput {
    /// Records an input packet and reports whether it should receive wheel
    /// easing. A pause of 250 ms starts a new gesture; fractional line deltas
    /// and pixel deltas select direct scrolling
    /// for the remainder of the gesture.
    pub fn should_animate(&mut self, delta: mouse::ScrollDelta, now: Instant) -> bool {
        let gap = self
            .last_event
            .map(|last| now.saturating_duration_since(last));
        if gap.is_none_or(|gap| gap >= Duration::from_millis(250)) {
            self.continuous = false;
        }
        let whole_steps = match delta {
            mouse::ScrollDelta::Lines { x, y } => {
                x.is_finite()
                    && y.is_finite()
                    && x.fract() == 0.0
            ...
            mouse::ScrollDelta::Pixels { .. } => false,
        };
        self.continuous |= !whole_steps;
...
const WHEEL_EASING_DURATION: Duration = Duration::from_millis(150);
...
impl State {
    ...
    fn absolute_offset(&self, bounds: Rectangle, content: Rectangle) -> Vector {
        Vector::new(
    ...
    fn queue_smooth_scroll(
        ...
        let origin = self.absolute_offset(bounds, content);
        let pending = self.smooth_motion.map_or(origin, |motion| motion.target);
        let axis_target = |current: f32, pending: f32, delta: f32, maximum: f32| {
            // Reversing the wheel should respond immediately instead of first
            // consuming the remaining travel in the old direction.
            let base = if delta * (pending - current) < 0.0 {
                current
            } else {
                pending
            };
            (base + delta).clamp(0.0, maximum.max(0.0))
        };
        let target = Vector::new(
            axis_target(origin.x, pending.x, delta.x, content.width - bounds.width),
            axis_target(origin.y, pending.y, delta.y, content.height - bounds.height),
        );
        if target == origin {
            self.smooth_motion = None;
            return false;
        }
        self.smooth_motion = Some(SmoothMotion {
            origin,
            target,
            started: now,
        });
    ...
    fn advance_smooth_scroll(
        ...
        let Some(motion) = self.smooth_motion else {
            return false;
        };
        ...
        let maximum = Vector::new(
            (content.width - bounds.width).max(0.0),
            (content.height - bounds.height).max(0.0),
        );
        let target = Vector::new(
            motion.target.x.clamp(0.0, maximum.x),
            motion.target.y.clamp(0.0, maximum.y),
        );
        let next = motion.origin + (target - motion.origin) * eased;
        let next = Vector::new(next.x.clamp(0.0, maximum.x), next.y.clamp(0.0, maximum.y));
        let previous = self.absolute_offset(bounds, content);
        self.offset_x = Offset::Absolute(next.x);
        self.offset_y = Offset::Absolute(next.y);
        if progress >= 1.0 || next == target {
            self.smooth_motion = None;
        }
        previous != next
    }
