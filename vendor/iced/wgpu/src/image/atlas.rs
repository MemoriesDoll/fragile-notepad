...
// The application mostly uploads small icons and About artwork. Start at 4 MiB
// instead of 16 MiB per window; the measured animated scene grows to 8 MiB.
// Existing layer growth and fragmentation still handle larger image sets.
...
pub struct Atlas {
    ...
    // Measurement and worker-owned uploads do not need this shared texture.
    // Allocate all required layers on the first upload, then retain and grow it.
...
impl Atlas {
    ...
    pub fn with_size(
        ...
        Atlas {
            size,
            max_layers: device.limits().max_texture_array_layers as usize,
            backend,
            storage: None,
            texture_layout,
            layers,
    ...
    pub fn standalone(&self, device: &wgpu::Device, size: u32) -> Self {
        Self::with_size(device, self.backend, self.texture_layout.clone(), size)
    }
    ...
    fn allocate(&mut self, width: u32, height: u32) -> Option<Entry> {
        ...
        if width > self.size || height > self.size {
            let mut fragments: Vec<entry::Fragment> = Vec::new();
            let original_layers = self.layers.len();
            let mut y = 0;
            ...
            while y < height {
                ...
                while x < width {
                    ...
                    let Some(allocation) = self.allocate(width, height) else {
                        // Nothing has been uploaded yet. Return every reserved
                        // region and discard new layers when any fragment fails.
                        for fragment in &fragments {
                            self.deallocate(&fragment.allocation);
                        }
                        self.layers.truncate(original_layers);
                        return None;
                    };
    ...
    fn can_add_layer(&self) -> bool {
        let next = self.layers.len() + 1;
        // Keep the GL array/cubemap workaround inside the requested limit too.
        let physical =
            next + usize::from(self.backend == wgpu::Backend::Gl && next.is_multiple_of(6));
        physical <= self.max_layers
    }
    ...
    fn grow(
        ...
        if amount == 0 && self.storage.is_some() {
            return;
        }
        ...
        let texture_view = new_texture.create_view(&wgpu::TextureViewDescriptor {
