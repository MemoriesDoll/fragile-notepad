...
pub struct Pipeline {
    ...
    sampler_bindings: Option<[wgpu::BindGroup; 2]>,
}
...
impl Pipeline {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat, backend: wgpu::Backend) -> Self {
        ...
        let constant_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: if immediate {
                &constant_entries[1..]
            } else {
                &constant_entries
            },
        });
        // Samplers are engine resources. With immediate transforms no per-layer
        // buffer is part of this binding, so every layer/window can share it.
        let sampler_bindings = immediate.then(|| {
            [&nearest_sampler, &linear_sampler].map(|sampler| {
                device.create_bind_group(&wgpu::BindGroupDescriptor {
                    label: Some("iced_wgpu::image shared sampler binding"),
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            immediate_size: if immediate {
                mem::size_of::<Uniforms>() as u32
            } else {
                0
            },
        });
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(if immediate {
                source
                    .replace(
...
impl Layer {
    fn new(device: &wgpu::Device, pipeline: &Pipeline) -> Self {
        let uniforms = pipeline.sampler_bindings.is_none().then(|| {
        ...
        let make_binding = |sampler: &wgpu::Sampler| {
            device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry {
                        ...
                        resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                            buffer: uniforms.as_ref().expect("uniform parameter path"),
                            offset: 0,
                            size: None,
                        }),
                    },
        ...
        let [nearest_layout, linear_layout] =
            pipeline.sampler_bindings.clone().unwrap_or_else(|| {
                [
                    make_binding(&pipeline.nearest_sampler),
                    make_binding(&pipeline.linear_sampler),
                ]
            });
    ...
    fn prepare(
        ...
        if let Some(uniforms) = &self.uniforms {
            let bytes = bytemuck::bytes_of(&self.parameters);
    ...
    fn render<'a>(&'a self, render_pass: &mut wgpu::RenderPass<'a>) {
        if self.uniforms.is_none() {
            render_pass.set_immediates(0, bytemuck::bytes_of(&self.parameters));
        }
