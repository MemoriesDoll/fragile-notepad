...
impl Cache {
    pub fn new(
        ...
        Self {
            #[cfg(feature = "image")]
            icons: Atlas::with_size(device, backend, layout.clone(), atlas::ICON_SIZE),
            ...
            raster: Raster {
                ...
                belt: wgpu::util::StagingBelt::new(
                    device.clone(),
                    crate::buffer::STAGING_CHUNK_SIZE,
                ),
            },
            #[cfg(feature = "svg")]
    ...
    pub fn load_image(
        ...
        match self.raster.cache.get_mut(handle).unwrap() {
            Memory::Host(image) => {
                ...
                let upload = crate::image::raster::Storage::upload(
                    &mut self.atlas,
                    &mut self.icons,
                    device,
                    &mut encoder,
                ...
                let Some((entry, storage)) = upload else {
    ...
    pub fn upload_raster(
        ...
        if let Memory::Device { entry, storage, .. } = memory {
            return Some((entry, storage.bind_group(&self.atlas, &self.icons)));
        }
        ...
        if image.len() < MAX_SYNC_SIZE || cfg!(target_arch = "wasm32") {
            let (entry, storage) = crate::image::raster::Storage::upload(
                &mut self.atlas,
                &mut self.icons,
                device,
                encoder,
    ...
    pub fn trim(&mut self) {
        #[cfg(feature = "image")]
        {
            ...
            self.raster
                .cache
                .trim(&mut self.atlas, &mut self.icons, |_bind_group| {
                    #[cfg(not(target_arch = "wasm32"))]
            ...
            match work {
                worker::Work::Upload {
                    ...
                    self.raster.cache.insert(
                        ...
                        Memory::Device {
                            entry,
                            storage: crate::image::raster::Storage::Dedicated(bind_group),
                            allocation,
                        },
                    );
                }
