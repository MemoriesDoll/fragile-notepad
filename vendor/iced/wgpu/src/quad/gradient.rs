...
impl Pipeline {
    ...
    pub fn new(
        ...
        constants_layout: Option<&wgpu::BindGroupLayout>,
    ) -> Self {
        #[cfg(not(target_arch = "wasm32"))]
        {
            let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("iced_wgpu.quad.gradient.pipeline"),
                bind_group_layouts: &constants_layout.map(Some).into_iter().collect::<Vec<_>>(),
                immediate_size: if constants_layout.is_none() {
                    std::mem::size_of::<quad::Uniforms>() as u32
                } else {
                    0
                },
            });
            ...
            let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
                ...
                source: wgpu::ShaderSource::Wgsl(quad::shader(
                    concat!(
