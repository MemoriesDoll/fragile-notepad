    ...
    B: graphics::Compositor,
{
    ...
    fn begin_warm_up_offscreen(
        ...
        viewport: &graphics::Viewport,
        background_color: Color,
    ) -> Result<(), compositor::OffscreenWarmUpError> {
        match (self, renderer) {
            (Self::Primary(compositor), Renderer::Primary(renderer)) => {
                compositor.begin_warm_up_offscreen(renderer, viewport, background_color)
            }
            (Self::Secondary(compositor), Renderer::Secondary(renderer)) => {
                compositor.begin_warm_up_offscreen(renderer, viewport, background_color)
            }
    ...
    fn poll_warm_up_offscreen(
        &mut self,
        renderer: &mut Self::Renderer,
    ) -> Result<Option<compositor::OffscreenWarmUpEvidence>, compositor::OffscreenWarmUpError> {
        match (self, renderer) {
            (Self::Primary(compositor), Renderer::Primary(renderer)) => {
                compositor.poll_warm_up_offscreen(renderer)
            }
            (Self::Secondary(compositor), Renderer::Secondary(renderer)) => {
                compositor.poll_warm_up_offscreen(renderer)
            }
