...
impl TextRenderer {
    ...
    pub fn prepare_with_depth<'a>(
        ...
        let vertices_valid = self.vertices_valid;
        // A failed prepare may overwrite part of the CPU vector without an
        // upload. Invalidate it until the complete new draw is encoded.
        self.vertices_valid = false;
        self.glyphs_to_render = 0;
        let mut changed = usize::MAX..0;
        ...
        for text_area in text_areas {
            ...
            for run in layout_runs {
                for glyph in run.glyphs.iter() {
                    ...
                    let index = self.glyphs_to_render as usize;
                    if let Some(previous) = self.glyph_vertices.get_mut(index) {
                        if *previous != vertex {
                            *previous = vertex;
                            changed.start = changed.start.min(index);
                            changed.end = index + 1;
                        }
                    } else {
                        self.glyph_vertices.push(vertex);
                        changed.start = changed.start.min(index);
                        changed.end = index + 1;
                    }
                    self.glyphs_to_render += 1;
                }
            }
        }

        self.glyph_vertices.truncate(self.glyphs_to_render as usize);
        self.upload_vertices(device, encoder, vertices_valid, changed);
        self.vertices_valid = true;
    ...
    fn upload_vertices(
        ...
        if self.vertex_buffer_size >= vertices_raw.len() as u64 {
            ...
            let stride = std::mem::size_of::<GlyphToRender>();
            let bytes = changed.start * stride..changed.end * stride;
            self.staging_belt
                .write_buffer(
                    encoder,
                    &self.vertex_buffer,
                    bytes.start as u64,
                    NonZeroU64::new(bytes.len() as u64).expect("Non-empty vertices"),
                )
                .copy_from_slice(&vertices_raw[bytes]);
        } else {
            // Earlier encoded draws can still reference the previous buffer.
            // Let submission ownership release it after those draws complete.
    ...
    pub fn render(
        ...
        if self.glyphs_to_render == 0 || !self.vertices_valid {
            return Ok(());
        }
