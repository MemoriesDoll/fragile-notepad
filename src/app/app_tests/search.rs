...
fn go_to_line_clamps_high_input_to_last_line() {
    ...
    let _ = app.update(Message::ToggleAdvancedSearch(
        crate::message::AdvancedSearchTab::GoToLine,
    ));
    let _ = app.update(Message::AdvancedSearchQueryChanged(String::from("999")));
    let _ = app.update(Message::AdvancedFindNextRun);
    ...
    assert_eq!(app.search_dialog.status, "Line 3 of 3");
}
...
fn go_to_line_zero_targets_first_line() {
    ...
    let _ = app.update(Message::AdvancedSearchQueryChanged(String::from("0")));
    let _ = app.update(Message::AdvancedFindNextRun);
    ...
    assert_eq!(app.search_dialog.status, "Line 1 of 3");
}
...
fn go_to_line_invalid_input_preserves_selection() {
    ...
    let _ = app.update(Message::AdvancedSearchQueryChanged(String::from("two")));
    let _ = app.update(Message::AdvancedFindNextRun);

    let document = app.workspace.active_document().expect("active document");
    assert_eq!(document.selection, selection);
    assert_eq!(app.search_dialog.status, "Invalid line number");
}
